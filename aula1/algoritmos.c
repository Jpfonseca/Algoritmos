...
int main (void)
{
	unsigned int Result, Line, N,control,  NLines; int Test;
	...
	for (control=0; control!=4; control++){
		for (Line = 1, N = 1; Line <= NLines; Line++, N *= 2)
		{
			...
			switch(control){
				case 0:Result = Algorithm1(N);break;
				case 1:Result = Algorithm2(N);break;
				case 2:Result = Algorithm3(N);break;
				case 3:Result = Algorithm4(N);break;			
			}
