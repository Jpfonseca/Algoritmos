...
PtMatrix MatrixCreate (unsigned int pnl, unsigned int pnc)
{
  ...
  if ((tmpMatrix=(PtMatrix) malloc (sizeof(struct matrix)))==NULL){
      Error=NO_MEM;
      return NULL;

  }
  if ((tmpMatrix->Matrix=(double**)calloc (pnl, sizeof(double*)))==NULL){
    Error=NO_MEM;
    free(tmpMatrix);
    return NULL;
  }
  ...
  for(I = 0; I < pnl; I++)
  {
    if((tmpMatrix->Matrix[I] = (double *) calloc(pnc, sizeof(double))) == NULL)
    { Error = NO_MEM;
      I--; 
      break;
    }
  ...
  tmpMatrix->NL=pnl; tmpMatrix->NC=pnc;
  Error=OK;
  return tmpMatrix;
}
...
void MatrixDestroy (PtMatrix *pmat)
{
  ...
  /*libertação do espaço ocupado pela matrix*/
  free(TmpMatrix);
...
PtMatrix MatrixTranspose (PtMatrix pmat)
{
  ...
  int i, j;
    ...
    if((MtTransp=MatrixCreate(pmat->NC ,pmat->NL))==NULL){
      Error=NO_MEM;
      return NULL;
    }
    for (i = 0; i < pmat->NL; i++)
    {
      for (j= 0; j < pmat->NC; j++)
      {
        MtTransp->Matrix[j][i]=pmat->Matrix[i][j];
      } 
    }
...
PtMatrix MatrixAddition (PtMatrix pmat1, PtMatrix pmat2)
{
  ...
  if((Addition=MatrixCreate(pmat1->NL, pmat1->NC))==NULL||(Addition=MatrixCreate(pmat2->NL, pmat2->NC))==NULL){
    Error=NO_MEM;
    return NULL;
  }
  ...
  for (i = 0; i < pmat1->NL; i++)
  {
    ...
    {
      Addition->Matrix[i][j]=pmat1->Matrix[i][j]+pmat2->Matrix[i][j];
    }
  }
  Error=OK;
  return Addition;
...
PtMatrix MatrixSubtraction (PtMatrix pmat1, PtMatrix pmat2)
{
  ...
  if ((Subtraction=MatrixCreate(pmat1->NC, pmat1->NL))==NULL||(Subtraction=MatrixCreate(pmat2->NC, pmat2->NL))==NULL)
  {
    Error=NO_MEM;
    return NULL;
  }
...
double MatrixDeterminant (PtMatrix pmat)
{
  ...
  if ((tmp=(double*) calloc ((pmat->NL)*(pmat->NC),sizeof(double)))==NULL)
  {
    Error= NO_MEM;
    return;
  }
  ...
  for ( l = 0; l < pmat->NL; l++)
  {
    for (m = 0; m < pmat->NC; m++)
    {
      tmp[i]=pmat->Matrix[l][m];
      i++;
    }
  }

  /* Usa funcao auxiliar para calculo do determinante da matriz */
  determinant=Determinant(tmp, pmat->NL, pmat->NC);
...
PtMatrix MatrixSubMatrix (PtMatrix pmat, unsigned int pl, unsigned int pc)
{
  ...
  for (l = 0; l < pmat->NL; l++)
  {
    if (pl!=l)
    {
      ...
      {
        if(pc!=j){
          SubMatrix->Matrix[i][p]=pmat->Matrix[l][j];
          p++;
        }
      }
      i++;
